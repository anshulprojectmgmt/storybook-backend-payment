...
export const createStoryBook = async (req, res) => {    
    const { title, description, cover_photo, page_count, age_group, min_required_photos, source, author } = req.body;
    try {
        ...
        res.status(201).json({ message: "Story book created successfully", ok: true });
    } catch (error) {
        res.status(500).json({ message: "Error creating story book" , ok: false });
    }
}
