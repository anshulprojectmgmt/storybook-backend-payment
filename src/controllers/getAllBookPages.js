...
export const getAllBookPages = async (req, res) => {
  try {
    ...
    const docs = await AiKidImageModel.find(
      { req_id, status: "completed" },
      { page_number: 1, image_urls: 1, front_cover_url: 1, back_cover_url: 1 }
    ).sort({ page_number: 1 });
    ...
    const pages = docs
      .filter((d) => d.image_urls?.length > 1)
      .map((d) => d.image_urls[1]); // ✔ Use margin version

    const front = docs.find((d) => d.front_cover_url)?.front_cover_url || null;
    const back = docs.find((d) => d.back_cover_url)?.back_cover_url || null;
